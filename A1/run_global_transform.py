...
def apply_transform(image, scale, rotation, translation_x, translation_y, flip_horizontal):
    ...
    center = np.array([transformed_image.shape[0]//2, transformed_image.shape[1]//2])
    T1 = np.eye(3)
    T1[0:2, 2] = -center
    T2 = np.eye(3)
    T2[0:2, 2] = center
    T = np.eye(3)
    T[0:2, 2] = np.array([translation_x, translation_y])
    S = np.diag([scale, scale, 1])
    R = np.array([[np.cos(2*np.pi*rotation/360), -np.sin(2*np.pi*rotation/360), 0],
                  [np.sin(2*np.pi*rotation/360), np.cos(2*np.pi*rotation/360), 0], [0, 0, 1]])
    A = T.dot(T2.dot(R.dot(S.dot(T1))))
