...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        J_proj[:, 0, 1] = K[0, 1] / cam_points[:, 2]
        J_proj[:, 0, 2] = -((K[0, 0] * cam_points[:, 0] + K[0, 1] * cam_points[:, 1]) / (cam_points[:, 2])**2)
    ...
    def compute_gaussian_values(
        ...
        ### gaussian = ... ## (N, H, W)
        P1 = torch.einsum('nhwm,nmm->nhwm', dx, torch.linalg.inv(covs2D))
        P2 = torch.einsum('nhwm,nhwm->nhw', P1, dx)
        gaussian = torch.exp(-0.5 * P2) / (2 * torch.pi * torch.sqrt(torch.linalg.det(covs2D)))
    ...
    def forward(
        ...
        weights[0, :, :] = alphas[0, :, :]
        weights[1:, :, :] = torch.cumprod(T, dim=0) * alphas[1:, :, :]
